...
uses 
{cthreads,} Classes, SysUtils, Dialogs, type_of_model, data, Menus, DOM, interpolations, Grid, MTProcs;
...
begin
  ...
  else if (model = 2) or (model = 4) then
  ...
  writeln('* Reading 3...');
  if (model = 4) then
    morpheo_input_read(XMLDocument1, model2input, Nodes2, Elements2)
  else
...
begin
  ...
  if (model = 1) then
    ...
    begin
      model1input := '../Examples-FEDES/Example1-ManufacturingProcessChain-2ndLoop/Process-1-Abaqus-Input.inp';
      model1output := '../Examples-FEDES/Example1-ManufacturingProcessChain-2ndLoop/Process-1-Abaqus-Output.dat';
      model2input := '../Examples-FEDES/Example1-ManufacturingProcessChain-2ndLoop/Process2-Machining-XML-input.vtu';
    end;
  ...
  WriteLn('Nodes1: ', length(Nodes1));
  WriteLn('Elements1: ', length(Elements1));
  WriteLn('Nodes2: ', length(Nodes2));
  WriteLn('Elements2: ', length(Elements2));

{
  writeln('* Num of threads: ', 1);
  writeln(stderr, '* Num of threads: ', 1);
  SetNumOfThreads(1); SetNumOfThreads2(1);
  calculate;
  writeln('* Num of threads: ', 2);
  writeln(stderr, '* Num of threads: ', 2);
  SetNumOfThreads(2); SetNumOfThreads2(2);
  calculate;
  writeln('* Num of threads: ', 4);
  writeln(stderr, '* Num of threads: ', 4);
  SetNumOfThreads(4); SetNumOfThreads2(4);
  calculate;
}
  writeln('* Num of threads: ', 8);
  writeln(stderr, '* Num of threads: ', 8);
  {SetNumOfThreads(8);} SetNumOfThreads2(8);
  calculate;
end.
