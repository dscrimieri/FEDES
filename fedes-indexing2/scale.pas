...
procedure InOrderVisit(Scale: BTreePtr);
  ...
  while (Idx <> Scale^.Index) and not err do
    begin
      if (Idx < Scale^.Index) and (Scale^.Left <> nil) then
        ...
        begin
          WriteLn('Error. Value not found in scale for index: ', Idx, ', last index encountered: ', Scale^.Index);
          err := true;
        end;
    end;
...
begin
  if Scale <> nil then
    begin
      InOrderVisit(Scale^.Left);
      Write(' (', Scale^.Value:2, ' ', Scale^.Index, ') ');
      InOrderVisit(Scale^.Right);
    end;
end;
